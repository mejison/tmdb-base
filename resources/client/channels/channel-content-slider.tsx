  ...
  return (
    <Fragment>
      ...
      <div className="gap-24 md:flex">
        ...
        {!isMobile && <UpNext titles={titles} activePage={activePage} />}
      </div>
    </Fragment>
  );
}
  ...
  return (
    <div className="relative h-full w-full flex-shrink-0 snap-start snap-normal overflow-hidden rounded">
      ...
      <div className="absolute inset-0 isolate flex h-full w-full items-center justify-start gap-24 rounded p-30 text-white md:items-end">
        <div className="absolute left-0 h-full w-full bg-gradient-to-b from-black/100 max-md:top-0 md:bottom-0 md:h-3/4 md:bg-gradient-to-t" />
        {!isMobile && (
          <TitlePoster
            title={item}
            size="max-h-320"
        ...
        <div className="z-10 text-lg md:max-w-620">
          ...
          {!isMobile && <p>{item.description.slice(0, 200)}</p>}
          {item.primary_video && (
            <Button
              variant="flat"
              color="primary"
