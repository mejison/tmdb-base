...
class Title extends BaseModel
{
    ...
    public function findSeason(int $number): Season|null
    {
        return $this->seasons()
            ->where('number', $number)
            ->firstOrFail();
    ...
    public function findEpisode(int $season, int $episode): Episode|null
    {
    ...
    protected function needsUpdating(): bool
    {
        // title was never synced from external site
        if (
            !$this->tmdb_id ||
            !$this->exists ||
            ($this->allow_update && !$this->fully_synced)
        ) {
            return true;
        }
